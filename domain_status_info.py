...
class DomainStatus(object):
    ...
    def get_ip(self, host):
        ...
        try:
            return ', '.join(socket.gethostbyname_ex(host_name)[2])
    ...
    def get_status_code(self, host):
        """Gets status code or Error message after a HTTP 'GET' request.
          ...
          Returns:
            string: http code  and reason if successful
        ...
        rand_headers = {'User-Agent': user_agent}
        try:
    ...
    def _status_code_helper(self, host):
        ...
        user_agent = self.get_random_user_agent()
        rand_headers = {'User-Agent': user_agent}
        req = urllibrary.Request(host, headers=rand_headers)
        try:
